                          ...
                          VelocityInterface* bvi, AngularDistributionInterface* adi, PolarizationProfileInterface* ppi)
{
    ...
    _W = L * lambda;
    _lambda0 = lambda;
    _D = std::sqrt(bfr.x()*bfr.x()+bfr.y()*bfr.y()+bfr.z()*bfr.z());
    _bvi = bvi;
    _adi = adi;
    _ppi = ppi;
    _compIndex = 0;
...
void PhotonPacket::launchEmissionPeelOff(const PhotonPacket* pp, Direction bfk)
{
    ...
    Direction dir = pp->direction();
    _D = pp->distance_traveled() + (-1)*(dir.x()*pos.x()+dir.y()*pos.y()+dir.z()*pos.z());
...
void PhotonPacket::propagate(const PhotonPacket* pp, double s)
{
    propagatePosition(s);
    _D = pp->distance_traveled() + s;
}
