...
void FluxRecorder::setLightcurveParameters(bool Active, int numBins, double Range)
{
    _lcactive = Active;
    _numBins = numBins;
    _range = Range;
...
void FluxRecorder::finalizeConfiguration()
{
    ...
    // define the lightcurve bins
    _lcrange = _range * Constants::c();
    _binSize = _lcrange / _numBins;
    ...
    _lc.resize(PrimaryScatteredLevel + (_numBins));

    for (int i = 0; i != _numBins; ++i){
        _lc[PrimaryScatteredLevel + i].resize(lenSED);
    }
...
void FluxRecorder::detect(PhotonPacket* pp, int l, double distance)
{
    ...
    _D = units->olength(pp->distance_traveled()) / units->olength(_binSize);

    if (_D<0) _D= _numBins;
    if (_D>(_numBins-1)) _D=_numBins-1;
    ...
    for (int ell : _lambdagrid->bins(wavelength))
    {
        ...
        if (_includeFluxDensity)
        {
            if (_recordTotalOnly)
            {
                ...
                LockFree::add(_lc[PrimaryScatteredLevel + _D][ell], Lext);
            }
            else
            {
...
void FluxRecorder::calibrateAndWrite()
{
    ...
    if (_includeFluxDensity)
    {
        ...
        if (_lcactive)
        {
            ...
            lcFile.addColumn("wavelength; " + units->swavelength(), units->uwavelength());
            vector<string> lcNames;
            vector<Array*> lcArrays;
            //Organize the data in columns
            for (int i = 0; i != _numBins; ++i)
            {
                // write the timesteps in days instead of the standard Gyr outUnit
                lcNames.push_back(std::to_string(i*units->otime(_range)/_numBins * (356e9)) + " days");
                lcArrays.push_back(&_lc[PrimaryScatteredLevel + i]);
            }
            ...
            for (int ell : Indices(numWavelengths, units->rwavelength()))
            {
                ...
                for (const Array* array : lcArrays) values.push_back((*array)[ell]);
